...
class TestAudioProcessor:
    ...
    def test_resample_same_rate(self):
        ...
        config = get_config()
        resampled = processor.resample(data, config.audio.sample_rate, config.audio.sample_rate)
...
class TestWebSocketBridge:
    ...
    async def test_bridge_initialization(self):
        ...
        test_url = "ws://localhost:8001/ws/voice"
        bridge = WebSocketBridge(test_url, 8080)
        assert bridge.ai_platform_url == test_url
